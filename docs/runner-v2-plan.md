...
> Status: **implementing** · Schema bump: FixingPlan `2.0 → 3.0` · Agent API: `runner.v1`
...
- **hook / safety card 決策**（v1 內建 AuditHook：allow + 完整審計；safety card 之後可 deny）
...
| **能力預檢 gate 歸零**：`validate_supported_commands` 失效，LLM 幻覺 argv 直接落地。 | v1 由 AuditHook（allow+audit）承接，safety card 待補。**顯式記錄為 trade-off**（見 §6）。 |
...
舊的 `command_id`-based `FixingPlan 2.0` schema 保留在檔案中標記 `LEGACY`，但 executor 不再接受。
...
- Hook pipeline：`before_run` → execute → `after_run` / `on_error`。`HookDecision = {decision, card_id, reason, annotations}`。v1 內建 `AuditHook`（永遠 allow），decision 與 argv hash 寫進 response 供 Layer 4 入審計。`pi-agent/src/safety_cards/` 目錄預留。
- **API 變更**：
...
// response
{ "status": "success|failed|timeout", "returncode": 0, "stdout": "...", "stderr": "...",
  "duration_ms": 12, "hook": { "decision": "allow", "card_id": "audit.allow_all.v1",
...
- `validate_plan`：3.0 欄位、`runner.argv` 非空、verification.runner.side_effect==read、expected 結構化且無 free-text。
...
- pi-agent：`test_catalog.py` → `test_runner.py`
  ...
  - AuditHook response 含 decision / context / argv hash。
- Layer 4：`test_executor_policy.py`
  - v3 plan 接受 runner specs、拒絕缺 `runner.argv`。
  - mutating（`side_effect=mutate`）取得 node lock。
  - retry 用 `idempotency.max_attempts`。
  - `apply_extractors` 正確產出結構化欄位；壞服務不被誤判 verified。
- Layer 2：`test_claude_style_plan.py` / `test_lab_node_agent_steps.py`
...
| 區塊 | 檔案 | 變更 |
|---|---|---|
| Schema 3.0 | `layer2-analyzer/src/schemas/action_plan.py` | 新增 `RunnerSpec` / `ExtractRule` / `RollbackSpec`；`VerificationSpec` 改 runner+extract；`FixingPlanStep` 改 runner+context+idempotency；`FixingPlan` 升 3.0；`StepCommand` 移除 `command_id`、加 `operation` |
...
| On-Device Agent | `pi-agent/src/main.py` | `CommandRunner` + `AuditHook` + `/v1/commands/run` + `runner_capabilities`；移除 catalog |
| Root runner | `pi-agent/wrappers/aads-root-command-runner`（新） | 讀 stdin JSON、`exec` argv（shell=False） |
| Layer 4 | `layer4-executor/src/executor.py` | schema 3.0；`run_runner_command`；`apply_extractors`；side_effect 鎖；rollback.runner |
| Dashboard | `dashboard/app.py` + `static/app/*` | `runner_capabilities`；前端改 runner label；schema gate 3.0 |
| DB | `layer0-storage/.../000_init_tables.sql` + `004_runner_capabilities.sql`（新） | `supported_commands` → `runner_capabilities` |
| Install | `pi-agent/install/install.sh`、`scripts/lab/deploy-target.sh` | 裝 root runner + 全 wrapper；sudoers 收斂成單一 root runner grant |

**單元測試**（44 passed）：`pi-agent/tests/test_runner.py`(7)、`layer4-executor/tests/test_executor_policy.py`(14)、
`layer2-analyzer/tests/test_claude_style_plan.py`+`test_lab_node_agent_steps.py`(23)。Lab 驗證（§8）仍待人工在 Ubuntu target 上執行。
